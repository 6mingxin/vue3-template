...
function getBreadcrumbMenuItem(activeKey: string, menu: GlobalMenuOption) {
  ...
  if (activeKey.includes(menu.routeName) && menu.children && menu.children.length) {
    breadcrumbMenu.push(menu)
    breadcrumbMenu.push(...menu.children.map(item => getBreadcrumbMenuItem(activeKey, item as GlobalMenuOption)).flat(1))
  }
...
function transformBreadcrumbMenuToBreadcrumb(menu: GlobalMenuOption, rootPath: string) {
  ...
  if (hasChildren) {
    breadcrumb.children = menu.children?.map(item => transformBreadcrumbMenuToBreadcrumb(item as GlobalMenuOption, rootPath))
  }
  return breadcrumb
}
